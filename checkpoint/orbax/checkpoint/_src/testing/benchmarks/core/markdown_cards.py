...
_SCORECARD_HEADLINE_KEYS: tuple[tuple[str, str, str], ...] = (
    # Keys carry the measure() operation prefix (save_blocking_,
...
def render_configuration(
  ...
  """Run configuration as readable markdown.
...
def render_aggregated_metrics(
  ...
  """mean / std / min / max / n / unit per metric, grouped by `/` prefix.

  The numbered section prefix (`2_save_breakdown/`, `3_load_breakdown/`, …)
  mirrors the Scalars-view navigation so a reader finds a metric the same
...
def options_to_hparams(options: Any) -> dict[str, bool | int | float | str]:
  ...
  HParams values must be primitives (bool / int / float / str). Anything
  else (None, list, tuple, nested) gets `str()`-ified so the run still
  appears in Parallel Coordinates rather than being dropped silently.
  """
