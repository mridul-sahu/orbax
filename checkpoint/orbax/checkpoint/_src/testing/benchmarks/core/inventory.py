...
def _classify_file(name: str) -> str:
  """Buckets a leaf filename into a format category for the breakdown table.

  ocdbt: per-process ocdbt files + the merged manifest.
  zarr3: zarr3 chunk + metadata files.
  metadata: orbax pytree / checkpoint / sharding metadata.
  other: anything else (custom blobs, future formats, …).
  """
...
def scan_checkpoint(path: epath.Path) -> CheckpointInventory:
  """Walks `path` recursively and returns counts + size totals.

  Returns an empty inventory if the path doesn't exist or can't be scanned;
  scan failures log a warning rather than raise so a benchmark that
  completed successfully doesn't get marked failed just because the
  post-save walk hit an IO error.
  """
  ...
  def _walk(p: epath.Path) -> None:
    nonlocal total_bytes, file_count, small_file_count, largest, smallest
    try:
      children = list(p.iterdir())
    except OSError as e:
      logging.warning("inventory: cannot iterdir %s: %s", p, e)
      return
    for child in children:
      ...
      if smallest < 0 or size < smallest:
        smallest = size
      bucket = _classify_file(child.name)
