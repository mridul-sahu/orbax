...
async def _read_first_metadata(
  ...
  for item in await async_path.iterdir(pending_dirs[0]):
    if not await async_path.is_dir(item):
      continue
    first_meta_path = item / format_utils.PYTREE_METADATA_FILE
...
async def _recursive_merge(src: epath.Path, dst: epath.Path):
  ...
  logging.warning(
      'File collision on %s during finalize. Overwriting destination file.',
      src.name,
  )
  if await async_path.is_dir(dst):
    await async_path.rmtree(dst)
  else:
    await async_path.unlink(dst)
  await async_path.rename(src, dst)


async def _merge_pytree_directory(
    pytree_src: epath.Path,
    pytree_dst: epath.Path,
    uuid_str: str,
):
  ...
  async def _merge_item(item_path: epath.Path):
    if item_path.name in [format_utils.PYTREE_METADATA_FILE, '_sharding']:
...
async def _merge_all(partial_path: epath.Path):
  ...
  for p_dir in pending_dirs:
    ...
    async def _process_item(item: epath.Path, uuid_str: str):
      ...
      elif await async_path.is_dir(item) and await async_path.exists(
          item / format_utils.PYTREE_METADATA_FILE
      ):
        pytree_directories.add(item.name)
        pytree_dst = partial_path / item.name
        await async_path.mkdir(pytree_dst, parents=True, exist_ok=True)
        await _merge_pytree_directory(item, pytree_dst, uuid_str)
      else:
  ...
  handler = base_pytree_checkpoint_handler.BasePyTreeCheckpointHandler(
      use_zarr3=use_zarr3
  )
